...
Screen review is just like it sounds. It lets you examine all text that NVDA can see on your visible screen. Oftern, text is there that NVDA cannot see.

Object review is also just like it sounds. It lets you review the text contained in the navigator object. As you move from Windows object to another, your text review commands operate only on the object the navigator object is pointing to.

Document review works only when you are in what NVDA calls a "browse mode" object, what JAWS calls a virtual cursor. You normally can only use the document review in a word processor, usually Word, or HTML pages such as a message in Outlook or a web page in your browser. There are exceptions, however. Many applications that use HTML will put you in a web-like interface that NVDA can navigate with document review.
...
Refers to the concept of keeping one pointer or cursor at the same position as another cursor. For eexample, the navigator object can be thether to the focus and usually is, but when you use object navigation, you are moving away from the focus. The review cursor is also usually tether to the focus, but when you use screen review, it also moves around. The same is true for text review. 
...
When you use the JAWS keystrokes to move to the next or previous line, word or character, you are using the equivalent of the NVDA text review. 
...
Master a handful at a time. For example, focus simply on text review and use it on a familiar document so you get the hang of how it works. A list of most common keystrokes both for text reviw and object navigation is given below with complete explanations of they work. 
...
Don't worry if you are completely confused. This is one feature set you use when you need it. For example, your buss is requiring you use an application that doesn't seen to be very keyboard accessible. Or you need to carefully review some messages a program is sending. In general, you would:
...
Here's a list formatted in ways that are a bit easier to read than in the official NVDA documentation. It also includes explanations about the patterns the keystrokes follow to make them a bit easier to remember.
...
So for example if you wanted to read the next word, you'd press the six. But if you wanted to go to the end of that line,you'd hold the shift and press the Three. If you wanted to move to a previous line, you'd use the seven key. But if you wanted to return back to the start of the text, you'd hold the shift and then hit the seven key.

Note too that the shift with the numeric keypad 4 and 6 is not defined. You can use the input gestures feature to assign them to a regularly used navigation command.
...
- Read the current character under the review cursor. -- NVDA+Period
  ...
  Remember too we are dealing with the review cursor here. If you are reading the current line that the system cursor is positioned on, the keystroke is Control+L. If text review is tethered to the system focus, then you will hear the same thing with NVDA+Shift+Period. 
...
- First child object -- NVDA+2
  ...
  If you press the NVDA key with the numberic keypad 4 you are moving in the same level of the hierarchy to the first object, and you are likely already there if you got there with the 2 key previously. But if you hold the NVDA key with the numeric keypad 6, you are moving on the same levelright to the next object on that level.

A good place to explore this is by loading up an application that has a ribbon or even better dialog boxes within a ribbon or menu structure.
...
If you press the Divide key, the third one from the right, it will click the left mouse button. If you hold the NVDA key and press the divede key, you can move the mouse pointer to the location of the navigator object. This is useful if you need to click on a control where you cannot get keyboard focus to work. So you'd first navigate there with object navigation. Then you'd hold the insert key which is the NVDA key and press the numeric keypad divide. Next you'd press the numeric keypad dived by itself to actually click on that control.
...
- 
- This is the same pattern as screen review except that objects can contain objects.A parent object may itself have a parent. A child object always has a parent object but it can also have siblings, that is multiple child objects you havigate left and right with the NVDA- Shift and right or left arrow keys. Remember in laptop mode the NVDA key is the caps lock key.
- 
